...
function thb_get_masonry_size($size = '', $thb_grid_type = 4) {
	...
	switch($size) {
		case 'large':
			...
			$class_image['image_size'] = $thb_grid_type == '3' ? 'werkstatt-squarelarge-3x' : 'werkstatt-squarelarge-3x';
			break;
		case 'wide':
			...
			$class_image['image_size'] = $thb_grid_type == '3' ? 'werkstatt-wide-3x' : 'werkstatt-wide-3x';
			break;
		case 'tall':
			...
			$class_image['image_size'] = $thb_grid_type == '3' ? 'werkstatt-tall-3x' : 'werkstatt-tall-3x';
			break;
		case 'small':
		default:
			...
			$class_image['image_size'] = $thb_grid_type == '3' ? 'werkstatt-squarelarge-2x' : 'werkstatt-squarelarge-2x';
			break;
	}
	return $class_image;
}
		...
		<?php } ?>
			...
			<?php
				 foreach ($categories as $cat) {
				 	...
				 	echo '<li><a href="#" data-filter=".thb-cat-' . esc_attr($term->slug) . '" data-count="'.esc_attr(count($posts)).'">' . esc_attr($term->name) . '</a></li>';
				 }
			?>
			</ul>
		</strong>
		...
		<div class="items custom_scroll portfolio-show-all" id="thb-show-all">
			...
			<?php
				 foreach ($portfolios as $portfolio) {
				 	...
				 	echo '<li><div>
				 		<figure style="background-image: url('.esc_attr($image_url[0]).')"></figure>
				 		<h6>' . $portfolio["title"] . '</h6>
				 		<aside class="cats">'.$portfolio["cats"] .'</aside>
				 		</div>
				 	</li>';
				 }
			?>
...
function thb_portfolio_nav() {
	...
	if (is_singular('post')) {
		...
		$portfolio_nav_exclude = ot_get_option( 'portfolio_nav_exclude', false);
		$in_same_term = $portfolio_nav_cat == 'on' ? true : false;
		$prev = get_adjacent_post( $in_same_term, $portfolio_nav_exclude, true, 'portfolio-category' );
		$next = get_adjacent_post( $in_same_term, $portfolio_nav_exclude, false, 'portfolio-category' );
	}
	if (
			...
			<div class="small-5 columns">
				<?php
					if ($prev) {
						...
						$image_link = wp_get_attachment_image_src($image_id, 'werkstatt-rectangle');
					?>
...
function thb_portfolio_attributes($style) {
	...
	<?php if ($client_name !== '' || $client_website !== '' || $client_date !== '' || $client_services !== '' || !empty($client_more)) { ?>
		<div class="folio__head--sub portfolio-attributes <?php if(isset($style)) { echo esc_attr($style); } ?>">
			<?php if ($client_name !== '') { ?>
			<div class="folio__sub folio folio__sub--client attribute"><strong><?php esc_html_e( /*'Client:', */'werkstatt' ); ?></strong> <?php echo esc_html($client_name); ?></div>
			<?php } ?>
			<?php if ($client_website !== '') { ?>
			<div class="folio__sub folio__sub--site attribute"><strong><?php esc_html_e( /*'Website:', */'werkstatt' ); ?></strong> <a href="<?php echo esc_url($client_website); ?>" title="<?php if ($client_name !== '') { echo esc_html($client_name); } ?>" target="_blank"><?php echo esc_html(thb_get_domain($client_website)); ?></a></div>
			<?php } ?>
			<?php if ($client_date !== '') { ?>
			<div class="folio__sub folio__sub--date attribute"><strong><?php esc_html_e( /*'Date:', */'werkstatt' ); ?></strong> <?php echo esc_html($client_date); ?></div>
			<?php } ?>
			<?php if ($client_services !== '') { ?>
			<div class="folio__sub folio__sub--cat attribute"><strong><?php esc_html_e( /*'Services:', */'werkstatt' ); ?></strong> <?php echo esc_html($client_services); ?></div>
			<?php } ?>
			<?php if (!empty($client_more)) { ?>
				<?php foreach ($client_more as $more ) { ?>
					<div class="folio__sub folio__sub--custom attribute">
						<strong><?php echo esc_attr($more['title']); ?>: </strong>
						<?php if ($more['client_custom_link'] !=='') { ?>
							<a href="<?php echo esc_html($more['client_custom_link']); ?>" title="<?php echo esc_attr($more['client_custom_value']); ?>" target="_blank">
						<?php } ?>
	...
	<div class="thb-portfolio-share">
		<strong>
			...
			<ul>
				...
				<li><a href="<?php echo 'http://pinterest.com/pin/create/link/?url=' . esc_url( $permalink ) . '&amp;media=' . ( ! empty( $image[0] ) ? esc_attr($image[0]) : '' ) . '&description='.htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8'); ?>" class="social pinterest" nopin="nopin" data-pin-no-hover="true"><i class="fa fa-pinterest"></i></a></li>
				<?php } ?>
				...
				<li><a href="<?php echo 'whatsapp://send?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . ' '. esc_url( $permalink ); ?>" class="whatsapp social" data-href="<?php echo esc_url( $permalink ); ?>" data-action="share/whatsapp/share"><i class="fa fa-whatsapp"></i></a></li>
				<?php } ?>
				<?php if (in_array('facebook-messenger',$sharing_type)) { ?>
					<?php $fb_app_id = ot_get_option( 'facebook_app_id'); ?>
				<li><a href="<?php echo 'fb-messenger://share/?link=' . esc_url( $permalink ) . '&app_id='.esc_url($fb_app_id); ?>" class="facebook-messenger social"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="96 93 322 324"><path d="M257 93c-88.918 0-161 67.157-161 150 0 47.205 23.412 89.311 60 116.807V417l54.819-30.273C225.449 390.801 240.948 393 257 393c88.918 0 161-67.157 161-150S345.918 93 257 93zm16 202l-41-44-80 44 88-94 42 44 79-44-88 94z" /></svg></a></li>
				<?php } ?>
				...
				<li><a href="<?php echo 'mailto:?subject=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&body='.urlencode( esc_url( $permalink ) ); ?>" class="email social"><i class="fa fa-envelope"></i></a></li>
				<?php } ?>
			</ul>
		</strong>
	</div>
...
function thb_portfolio_share_inline($widget_title,$sharing_type, $thb_alignment, $style = 'style1') {
	$id = get_the_ID();
	$permalink = get_permalink($id);
	$title = the_title_attribute(array('echo' => 0, 'post' => $id) );
	$image_id = get_post_thumbnail_id($id);
	...
	<div class="<?php echo esc_attr(implode(' ', $class)); ?>">
		...
		<ul>
			<?php if (in_array('facebook',$sharing_type)) { ?>
			<li><a href="<?php echo 'http://www.facebook.com/sharer.php?u=' . urlencode( esc_url( $permalink ) ).''; ?>" class="<?php echo esc_attr($icon_class); ?> facebook"><i class="fa fa-facebook"></i></a></li>
			<?php } ?>
			<?php if (in_array('twitter',$sharing_type)) { ?>
			<li><a href="<?php echo 'https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( $permalink ) ) . '&via=' . urlencode( $twitter_user ? $twitter_user : get_bloginfo( 'name' ) ) . ''; ?>" class="<?php echo esc_attr($icon_class); ?> twitter"><i class="fa fa-twitter"></i></a></li>
			<?php } ?>
			<?php if (in_array('pinterest',$sharing_type)) { ?>
			<li><a href="<?php echo 'http://pinterest.com/pin/create/link/?url=' . esc_url( $permalink ) . '&amp;media=' . ( ! empty( $image[0] ) ? $image[0] : '' ) . '&description='.htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8'); ?>" class="<?php echo esc_attr($icon_class); ?> pinterest" nopin="nopin" data-pin-no-hover="true"><i class="fa fa-pinterest"></i></a></li>
			<?php } ?>
			<?php if (in_array('linkedin',$sharing_type)) { ?>
			...
			<?php } ?>
			<?php if (in_array('whatsapp',$sharing_type)) { ?>
			<li><a href="<?php echo 'whatsapp://send?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . ''; ?>" class="<?php echo esc_attr($icon_class); ?> whatsapp" data-href="<?php echo esc_url( $permalink ); ?>" data-action="share/whatsapp/share"><i class="fa fa-whatsapp"></i></a></li>
			<?php } ?>
			<?php if (in_array('facebook-messenger',$sharing_type)) { ?>
				<?php $fb_app_id = ot_get_option( 'facebook_app_id'); ?>
			...
			<?php } ?>
			<?php if (in_array('email',$sharing_type)) { ?>
			<li><a href="<?php echo 'mailto:?subject=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&body='.urlencode( esc_url( $permalink ) ); ?>" class="<?php echo esc_attr($icon_class); ?> email"><i class="fa fa-envelope"></i></a></li>
			<?php } ?>
		</ul>
	</div>
	<?php
}
add_action( 'thb_portfolio_share_inline', 'thb_portfolio_share_inline', 3, 4 );
...
function thb_portfolio_video() {
	...
	$url = explode( '?', $portfolio_header_video_url);
	$portfolio_header_video_url = $url[0];
...
// Floating Portfolio.
function thb_floating_portfolio() {
	if ( 'off' === ot_get_option( 'floating_portfolio', 'off' ) ) {
		return;
	}
	$floating_portfolio_home     = ot_get_option( 'floating_portfolio_home', 'on' );

	if ( 'on' === $floating_portfolio_home && !is_front_page() ) {
		return;
	}
	$floating_portfolio_position = ot_get_option( 'floating_portfolio_position', 'left' );
	$floating_portfolio_list     = ot_get_option( 'floating_portfolio_list' );
	?>
	<aside class="thb-floating-portfolio <?php echo esc_attr( $floating_portfolio_position ); ?>">
		<div class="slick thb-floating-portfolio-slider" data-fade="true" data-columns="1" data-autoplay="true">
			<?php
			if ( $floating_portfolio_list ) {
				foreach ( $floating_portfolio_list as $portfolio ) {
					$thb_id         = $portfolio['portfolio'];
					...
					if ($main_listing_type === 'lightbox') {
						...
						$permalink = get_the_permalink( $thb_id );
					}
				?>
					<div class="thb-floating">
						<figure><?php echo wp_get_attachment_image( $image_id, 'werkstatt-square-2x' ); ?></figure>
						<div class="thb-floating-content">
							<?php if ( $thb_categories ) { ?>
								<aside class="thb-categories"><?php echo esc_html( $thb_categories ); ?></aside>
							<?php } ?>
							<h6><?php echo esc_html( get_the_title( $portfolio['portfolio'] ) ); ?></h6>
							<div class="thb-floating-progress"><span></span></div>
						</div>
						<a href="<?php echo esc_url( $permalink ); ?>" class="<?php echo esc_attr(implode(' ', $link_class)); ?>"></a>
					</div>
				<?php } ?>
			<?php } else { ?>
				<div class="thb-floating" style="padding: 20px;">
					<?php esc_html_e( 'Please select portfolio items for Floating Portfolio inside Theme Options', 'werkstatt' ); ?>
				</div>
			<?php } ?>
		</div>
	</aside>
...
add_action( 'thb_wrapper_end', 'thb_floating_portfolio' );
