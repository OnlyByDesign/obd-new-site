...
<?php
$thb_id             = get_the_ID();
$enable_pagepadding = get_post_meta( $thb_id, 'enable_pagepadding', true );
$classes[]          = 'folio post portfolio-detail';
...
<?php if ( post_password_required() ) { get_template_part( 'inc/templates/password-protected' ); } else { ?>
	<article itemscope itemtype="http://schema.org/Article" <?php post_class($classes); ?> role="article">
		<?php

			$portfolio_header_style = get_post_meta( $thb_id, 'portfolio_header_style', true ) ? get_post_meta( $thb_id, 'portfolio_header_style', true ) : 'style1';
			$style6_header_alignment = get_post_meta( $thb_id, 'style6_header_alignment', true ) ? get_post_meta( $thb_id, 'style6_header_alignment', true ) : 'right';
